...
#if defined(NDEBUG)
# error "PayCash cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x4c3832fec7fc007f1bfc71d9b038b43557126220c543271e5c93f948818454c1");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // PayCash: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "PayCash Signed Message:\n";
...
{
    // PayCash: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // PayCash: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // PayCash: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // PayCash: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // PayCash: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // PayCash: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe8136a9c1f28b8df05d82e1b8af35d86ffb675806f1d43fd9d6884434806d9c9");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "(Message from-2019) In 2030 Elon Musk will drink Coca Cola on Mars and pay with PayCash";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04da0af6c313fa2b3b220bb47b1f0008357339066c2631e8006fd5b5e922ac0c3627564168bb7521fe36b82630250d9c80266950e61610a7ad9a7700dbb6956768") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1555362645;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085076421;

        if (fTestNet)
        {
            block.nTime    = 1555362616;
            block.nNonce   = 385314011;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x1b6e9af624380b28a57368b5afb98089da36a4ecf0ba49b659323c0f12b98a23"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc2, 0xbc, 0xd6 }; // PayCash: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PayCashMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PayCashMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PayCashMiner(CWallet *pwallet)
{
    printf("PayCashMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("paycash-miner");
    ...
    try { loop {
        ...
        printf("Running PayCashMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PayCashMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PayCashMiner, pwallet));
}
